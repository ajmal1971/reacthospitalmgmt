...
function CreateOrEditDiagnosis() {
  ...
  const submitForm = async (data) => {
    ...
    try {
      if (switchData) {
        ...
        diagnosesService
          .createDiagnosis({ ...data, DepartmentId: selectedDepartment.$id })
          .finally(() => setIsLoading(false))
          .then((res) => {
            if (res) {
      ...
      if (res.documents) {
        ...
        if (switchData) {
          setSelectedDepartment(
            res.documents.find(
              (item) => item.$id === switchData?.department.$id,
            ),
          )
        }
      }
              ...
              <div className="flex flex-wrap">
                ...
                <div className="w-full md:w-1/2 px-2">
                  <Input
                    ...
                    className="mb-4"
                    {...register('LabAddress', { required: true })}
                  />
                </div>
              </div>
