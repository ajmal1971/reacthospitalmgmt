            ...
            <ul key={navItem.name} className="relative list-none">
                {navItem.children.length > 0 ? (
                    <li className="relative">
                        ...
                        {dropdownOpen && (
                        <ul className="absolute right-0 w-full mt-2 origin-top-right rounded-md shadow-lg md:w-48 list-none">
                            <li className="px-2 py-2 bg-white rounded-md shadow dark-mode:bg-gray-800">
                    ...
                    <li className='hover:bg-gray-200 rounded-md'>
                        <button key={navItem.name} onClick={() => navigateToPath(navItem.path)} className={`${!navItem.isChild ? 'px-4' : ''} py-2 mt-2 text-sm font-semibold bg-transparent rounded-lg dark-mode:bg-transparent dark-mode:focus:bg-gray-600 dark-mode:focus:text-white dark-mode:text-gray-200 md:mt-0 md:ml-1 hover:text-gray-900 focus:text-gray-900 focus:bg-gray-200 focus:outline-none focus:shadow-outline`}>
                        {navItem.name}
                        </button>   
                    </li>
                )}
